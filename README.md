...
## 2. 日期與時間增減功能
- **開始與結束日期/時間輸入框**：使用者可以手動輸入，或使用快捷鍵及光棒（`Ctrl + ↑` 或 `Ctrl + ↓`）來增減日期或時間。


## 3. 統計計算
- **計算平均值**：計算指定日期時間範圍內的數值欄位平均值。
- **計算電力啟停周期**：
  - 計算 `P(W)` 大於等於 闕值 的啟停周期次數。
  - 計算大於等於 闕值 和小於 闕值 的平均持續時間。
  - 計算大於等於 闕值 的百分比。
- **計算電力消耗**：
  - 計算 `WP(Wh)` 欄位的差值。
  - 使用線性法推算 24 小時的電力消耗。
...
## 9. 其他特定功能程式
- **general_plot.py**：通用數據繪圖工具，支援靈活的欄位選擇，可繪製折線圖和常態分佈圖
- **heatingrate.py**：加熱速率分析工具
- **onoff_calculate.py**：壓縮機開關狀態計算工具
- **temp_combine_rpm.py**：溫度與轉速數據合併工具
- **冷速.py**：冷卻速率分析工具
